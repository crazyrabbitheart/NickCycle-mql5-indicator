...
input bool shortTrigger = false;       // Short Trigger
...
//input ENUM_WHENNOTIFY WhenNotify;          // Notify
//
...
//int in_i_snr_line_width = 2; // Support Line Width
//int in_i_sup_line_width = 2; // Support Line Width
//int in_i_res_line_width = 2; // Resistance Line Width
...
int OnInit() {
   ...
   if(in_d_base_line)
   in_d_gap_of_levels = in_d_gap_of_levels * 10 * SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE);
...
//   }   
   ...
   for(; i<rates_total && !_StopFlag; i++) {
      bufferSnrS3[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * 3, Digits());
      bufferSnrS2[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * 2, Digits());
      bufferSnrS1[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * 1, Digits());
      bufferSnrMP[i] = NormalizeDouble(in_d_base_line, Digits());
      bufferSnrR1[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * -1, Digits());
      bufferSnrR2[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * -2, Digits());
      bufferSnrR3[i] = NormalizeDouble(in_d_base_line + in_d_gap_of_levels * -3, Digits());
      ...
      if (in_session){
         ...
         if (i > 0)
            if (close[i] > close[i-1] && (bufferRsi[i]>=30 || bufferRsi[i]<=70)){
               if (close[i] < bufferEma[i] || (close[i] > bufferEma[i] && open[i] < bufferEma[i])){
                  for (int j=0; j<ArraySize(bufferSR); j++){
                     if (open[i] < bufferSR[j] && close[i] > bufferSR[j]){
                        ...
                        //ObjectSetInteger(0, ArrowUpName, OBJPROP_WIDTH, 15 * pipsize);
                        //if(i == rates_total - 1){
               ...
               if (close[i] > bufferEma[i] || (close[i] < bufferEma[i] && open[i] > bufferEma[i])){
                  for (int j=0; j<ArraySize(bufferSR); j++){
                     if (open[i] > bufferSR[j] && close[i] < bufferSR[j]){
                        ...
                        //ObjectCreate(0, ArrowDownName, OBJ_ARROW_DOWN, 0, time[i], high[i] + 10 * pipsize );
                        //ObjectSetInteger(0, ArrowDownName, OBJPROP_COLOR, clrRed);
                        //ObjectSetInteger(0, ArrowDownName, OBJPROP_STYLE, STYLE_SOLID);
                        //ObjectSetInteger(0, ArrowDownName, OBJPROP_WIDTH, 15 * pipsize);
                        //if(i == rates_total - 1){
                        //   NotificationLevel = bufferSR[j];
                        //   Notify("sell");
                        //}
               ...
               }else if (shortTrigger && close[i] < bufferEma[i] && open[i] < bufferEma[i]){
                  for (int j=0; j<ArraySize(bufferSR); j++){
                     if (open[i] > bufferSR[j] && close[i] < bufferSR[j]){
                        string ArrowUpName = "ArrowUp" + IntegerToString(i);
                        ObjectCreate(0, ArrowUpName, OBJ_ARROW_UP, 0, time[i], low[i] - 3 * pipsize);
                        ObjectSetInteger(0, ArrowUpName, OBJPROP_COLOR, clrBrown);
                        ObjectSetInteger(0, ArrowUpName, OBJPROP_STYLE, STYLE_SOLID);
                        ObjectSetInteger(0, ArrowUpName, OBJPROP_WIDTH, 15 * pipsize);
                        if(i == rates_total - 1){
                           NotificationLevel = bufferSR[j];
         ...
         if(bufferTema[i] > 0 && bufferTema[i-1] < 0){
            //string line_name = "buyVline" + IntegerToString(i);
            //ObjectCreate(0, line_name, OBJ_VLINE, 0, time[i], 0);
         ...
         }else if(bufferTema[i] < 0 && bufferTema[i-1] > 0){
            //string line_name = "sellVline" + IntegerToString(i);
            //ObjectCreate(0, line_name, OBJ_VLINE, 0, time[i], 0);
...
//    
    ...
    string AlertText = "";
    if (type == "buy") 
      AlertText += "Price is in Danger Zone of " + DoubleToString(NotificationLevel, _Digits);
    else if (type == "sell") 
      AlertText += "Price is in Safe Zone from " + DoubleToString(NotificationLevel, _Digits);
