...
input double in_d_base_line = 17000.0;  // Base Level
...
input group "--- [5] Notification ---";
input bool in_b_email_alert = true;        // Email Alert
input bool in_b_sms_alert = true;          // SMS Alert
input bool in_b_sound_alert = true;        // Sound Alert
               ...
               if (close[i] < bufferEma[i] || (close[i] > bufferEma[i] && open[i] < bufferEma[i])){
                  for (int j=0; j<ArraySize(bufferSR); j++){
                     if (open[i] < bufferSR[j] && close[i] > bufferSR[j]){
                        ...
                        ObjectCreate(0, ArrowUpName, OBJ_ARROW_BUY, 0, time[i], low[i] - 3*SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE));
               ...
               if (close[i] > bufferEma[i] || (close[i] < bufferEma[i] && open[i] > bufferEma[i])){
                  for (int j=0; j<ArraySize(bufferSR); j++){
                     if (open[i] > bufferSR[j] && close[i] < bufferSR[j]){
                        ...
                        ObjectCreate(0, ArrowDownName, OBJ_ARROW_SELL, 0, time[i], high[i] + 3*SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE));              
                        ObjectSetInteger(0, ArrowDownName, OBJPROP_COLOR, clrBlue);
...
void Notify(string type){
    if ((!in_b_email_alert) && (!in_b_sms_alert) && (!in_b_sound_alert)) return;
