...
input bool showTimeLine = false;       // Show Time Line
...
//   }
   ...
   for(; i < rates_total && !_StopFlag; i++) {   
      ...
      if (hoursDifference <= setDayforShowTimeLine){
         double bufferSR[8];
         //if (autoSRLine) {
         ...
         if (in_session){
            ...
            if (i > 0){
               if (shortTrigger && !soldier){
                  ...
                  if (close[i] > close[i-1] && (bufferRsi[i]>=rsiforsoldier || bufferRsi[i]<=70)){
                     if (close[i] > bufferEma[i] || (close[i] > bufferEma[i] && open[i] < bufferEma[i])){
                        if(applySR){
                           for (int j=0; j<ArraySize(bufferSR); j++){
                              if (open[i] < bufferSR[j] && close[i] > bufferSR[j]){
                  ...
                  }else if (close[i] < close[i-1] && (bufferRsi[i]>=30 || bufferRsi[i]<=rsiforsoldier)){
                     if (close[i] < bufferEma[i] || (close[i] < bufferEma[i] && open[i] > bufferEma[i])){
                        if(applySR){
                           for (int j=0; j<ArraySize(bufferSR); j++){
                              if (open[i] > bufferSR[j] && close[i] < bufferSR[j]){
